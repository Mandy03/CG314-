...
void setup() {
  ...
  stampIcon=loadImage("stampIcon.png");
  ...
  file = minim.loadFile("music_box.mp3");
  file.loop();
}
...
void toolBoxSel() {
  ...
  if (canDraw) {
    ...
    if (mousePressed) {
      ...
      if (toolList == 6 ) {//Space
        image(stampIcon, mouseX, mouseY, 100, 100);
      }
    }
  }
}
...
void controllers() {
  ...
  if (mousePressed) {
    ...
    //space
    else if (mouseX > width-500 && mouseX < width &&
      mouseY > height-30 && mouseY <height) {   
      toolList=6;
      //print("here");
    }
